...
%install
cd $HOME
mkdir -p rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64
ls -al rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64
pwd
mkdir rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64/tmp
ls -al rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64
cp rpmbuild/SOURCES/server-%{version}.tar.gz rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64/tmp/
cp rpmbuild/SOURCES/web-%{version}.tar.gz rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64/tmp/
cd rpmbuild/BUILDROOT/genesis-%{name}-%{version}-1.amzn2.x86_64/tmp/
ls
pwd
